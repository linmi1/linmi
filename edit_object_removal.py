...
def removal_setup(opt, model_path, iteration, views, gaussians, pipeline, background, classifier, selected_obj_ids,
                  cameras_extent, removal_thresh):
    with torch.no_grad():
        ...
        for i in range(0, 256):
            mask = prob_obj3d[i, :, :]
            if torch.sum(mask).item() > 100:
                print(i, "is one of the ids")
                obj_ids.append(i)
        for id in obj_ids:
            selected_obj_ids = [id]
            ...
            masks.append([id, mask3d])

    # fix some gaussians
    # gaussians.removal_setup(opt,mask3d) # 88735
...
def removal(dataset: ModelParams, iteration: int, pipeline: PipelineParams, skip_train: bool, skip_test: bool,
            opt: OptimizationParams, select_obj_id: int, removal_thresh: float):
    ...
    gaussians = removal_setup(opt, dataset.model_path, scene.loaded_iter, scene.getTrainCameras(), gaussians, pipeline,
                              background, classifier, select_obj_id, scene.cameras_extent, removal_thresh)

    # 3. render new result
    scene = Scene(dataset, gaussians, load_iteration='_object_removal/iteration_' + str(scene.loaded_iter),
                  shuffle=False)
    with torch.no_grad():
        if not skip_train:
...
if __name__ == "__main__":
    ...
    removal(model.extract(args), args.iteration, pipeline.extract(args), args.skip_train, args.skip_test,
            opt.extract(args), args.select_obj_id, args.removal_thresh)
