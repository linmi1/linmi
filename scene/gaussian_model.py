...
class GaussianModel:
    ...
    def save_ply(self, path, masks=None):
        from move.move_with_mask import move
        from move.get_dir import get_dirs

        if masks == None:
            xyz = self._xyz.detach().cpu().numpy()
        else:

            images_bin_path = "/home/vision/work/linmi2/gaussian_grouping/data/color_gupai/distorted/sparse/0/images.bin"
            cameras_bin_path = "/home/vision/work/linmi2/gaussian_grouping/data/color_gupai/distorted/sparse/0/cameras.bin"
            dir1, dir2 = get_dirs("frame_Camera_1.png", "frame_Camera_26.png", images_bin_path, cameras_bin_path)
            check_image = "frame_Camera_3.png"

            # dir1是俯视图，dir2是右视图
            xyz = self._xyz.detach().cpu().numpy()
            for id, mask in masks:
                if id in [22]:
                    xyz = move(xyz, mask, dir1)
                # if id in [0]:
                #     xyz = move(xyz, mask, dir2)
